...
@patch("ploigos_step_runner.step_implementers.shared.gradle_test_reporting_mixin.get_plugin_configuration_absolute_path_values")
class TestGradleTestReportingMixin__attempt_get_test_report_directory(BaseStepImplementerTestCase):
    @staticmethod
    def __get_gradle_test_reporting_mixin():
        # create mixin object
    ...
    def test_one_found_result(self, get_plugin_configuration_absolute_path_values_mock):
        ...
        gradle_test_reporting_mixin = self.__get_gradle_test_reporting_mixin()
    ...
    def test_plugin_not_found(self, get_plugin_configuration_absolute_path_values_mock):
        ...
        with self.assertRaisesRegex(
            StepRunnerException,
            r"Error getting configuration \(mock-reports-dir-config-key\) from gradle"
            r" plugin \(mock-gradle-test-plugin\):"
...
class TestGradleTestReportingMixin__collect_report_results(unittest.TestCase):
    def test_give_dir_with_single_file_find_all_attributes(self):
        with TempDirectory() as test_dir:
            ...
            # run test
            report_results, warnings = GradleTestReportingMixin._collect_report_results(
    ...
    def test_multiple_test_suite_elements(self):
        with TempDirectory() as test_dir:
            ...
            # run test
            actual_results, actual_warnings = GradleTestReportingMixin._collect_report_results(
                test_report_dirs=[os.path.join(test_dir.path, 'test_result1.xml')]
            )
